...
""" TESTER: perform benchmarks and tests on your PC """
...
from hal.desktop import computer
...
class EightQueenTest(object):
    ...
    def run(self):        
        ...
        if computer.User.answer('Are you sure you want to proceed?'):
            max_board = self.size
            ...
            computer.Document.new(benchmark, str(id(benchmark)) + '.conf', os.getcwd())
            print('Just one extra moment of patience .. I\'ll save a configuration file..\n' \
                  'Done! The configuration file is \'' + os.path.join(os.getcwd(), str(id(benchmark)) + '.conf') + '\'')
        else:
            print('Now exiting!')
            sys.exit(0)
