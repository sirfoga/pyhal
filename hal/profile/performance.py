...
class EightQueenTest(object):
    ...
    def introduction():
        ...
        output = "So, let\"s get into the details.. I'm going to solve the classic Eight Queens Puzzle: " \
                 "it's the problem of placing eight chess queens on an 8×8 chessboard so that " \
                 "no two queens threaten each other.\n" \
                 "Finding all solutions to the eight queens puzzle is a good example of a nontrivial problem. " \
                 "For this reason, it is often used as an example problem for various programming techniques, " \
                 "including non-traditional approaches such as constraint programming, logic programming or " \
                 "genetic algorithms. Most often, it is used as an example of a problem that can be solved with a " \
                 "recursive algorithm (read more here: https://en.wikipedia.org/wiki/Eight_queens_puzzle) ... " \
                 "and since recursion involves both RAM storage and CPU use of your machine, this problem is a good " \
                 "tester to test your PC speed! Now let\'s get started.."
        return output
    ...
    def _run(self):
        ...
        for size in range(max_board_size + 1):
            ...
            self.update_std_out_and_log(
                "BOARD SIZE".ljust(10) + str(size).ljust(10) + "TIME REQUIRED (s)".ljust(20) + str(
                    '{:03.3f}'.format(timing)))

        finish_time = time.time()
        length = finish_time - start_time

        self.update_std_out_and_log("It took me " + str(length) + " seconds to complete all " + str(
            max_board_size) + " problems.\nThanks for your patience!")
        if input("Do you wanna save the results? [y/n]").startswith("y"):
            conf_file = os.path.join(os.getcwd(), "EightQueenTest" + str(int(random.random() * 10000)) + ".conf")
